...
Two feature branches run in parallel:
- **`feat/ai-guide`** (owner) — the guide + the personalization data it needs. Owner owns all migrations.
- **`feat/questionnaire-recommendations`** (collaborator) — the questionnaire, redefined as a **RIASEC personality test** whose only output is `profiles.personality_type` (`realistic|investigative|artistic|social|enterprising|conventional`), used to recommend catalog careers.

## Round 1 — Sign‑up data foundation (IN PROGRESS on `feat/ai-guide`)
...
## Later rounds (deferred)

- **Round 2 — Goal setting.** Store a goal as a human‑readable **title** + optional links to catalog rows. Entry points: a **Guide** button in `career.tsx` header (next to the heart) and a text box on the Guide tab.
